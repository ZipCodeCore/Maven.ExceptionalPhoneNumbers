...
public final class PhoneNumberFactory {
    ...
    public static PhoneNumber[] createRandomPhoneNumberArray(int phoneNumberCount) {
        PhoneNumber[] phNumbers = new PhoneNumber[phoneNumberCount];

        for(int i =0; i < phNumbers.length; i++){
            phNumbers[i] = createRandomPhoneNumber();
        }
        return phNumbers;
    }
    ...
    public static PhoneNumber createPhoneNumberSafely(int areaCode, int centralOfficeCode, int phoneLineCode) {
        StringBuilder phoneNumberConcat = new StringBuilder();
        phoneNumberConcat.append("(" + areaCode + ")-" + centralOfficeCode + "-" + phoneLineCode);

        try {
            return createPhoneNumber(phoneNumberConcat.toString());
    ...
    public static PhoneNumber createPhoneNumber(String phoneNumberString) throws InvalidPhoneNumberFormatException {
        logger.info("New number with value " + phoneNumberString);
